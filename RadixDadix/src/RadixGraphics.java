...
public class RadixGraphics extends JPanel {//24
    ...
    /**Places an array to begin with. */
    private void placeArray(Point p){
        var list = pieces;
        ///We start with our list then get a dummy image to test out the font.
        BufferedImage dummyImage = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
        Graphics2D actx = dummyImage.createGraphics();
        actx.setFont(FONT);

        ///After getting the font data, we can place down the items.
        FontMetrics fm = actx.getFontMetrics();
        int y_offset = fm.getAscent();
        int x = p.x;
        int y = p.y + y_offset;
        ///We get a feel for where to draw these items and create boxes.
    ...
    public void lsd(){
        ...
        for(int d = 0; d < max_d; d++){
            ...
            ///We place the values into the holder based on what digit it is.
            ...
            ///We visualize each holder.
            for (var holder : holders) {
                holder.spaceOut();
            }
            ///We clear the elements and place the holder items back into the main list and repeat this until sorted.
            main.elements.clear();
            for (var holder : holders) {
    ...
    /**Recursive helper function for regular msd. */
    private void recursive_msd(int d, Flexbox main){
        ...
        ///We create local slices to split on later.
        ...
        ///We store local flexers to remove flexers so we can use them for delineation.
        ...
        ///We loop through each local slice to find where we need to sort further.
